...
int main(void)
{
    ...
    clock_setup();          // Primero el reloj del sistema
console_setup(115200);  // Luego la consola UART
sdram_init();           // Luego SDRAM
lcd_spi_init();         // Luego la LCD
gfx_init(lcd_draw_pixel, 240, 320);
    ...
    /* Mensajes iniciales en pantalla LCD */
    gfx_setCursor(10, 10);
    gfx_puts("Dual HY-SRF05");
    gfx_setCursor(10, 40);
    gfx_puts("Iniciando...");
    lcd_show_frame();
    ...
    while (1) {
        ...
        gfx_puts("Medicion");
        gfx_setCursor(10, 40);
        snprintf(buf, sizeof(buf), "%lu", measurement_count);
        gfx_puts(buf);

        gfx_setCursor(10, 70);
        if (dist1 < 0)
            gfx_puts("Sensor 1: SIN LECTURA");
        else {
            gfx_puts("Sensor 1: ");
            gfx_setCursor(10, 100);
            snprintf(buf, sizeof(buf), "%.2f cm", dist1);
            gfx_puts(buf);
        }

        gfx_setCursor(10, 130);
        if (dist2 < 0)
            gfx_puts("Sensor 2: SIN LECTURA");
        else {
            gfx_puts("Sensor 2: ");
            gfx_setCursor(10, 160);
            snprintf(buf, sizeof(buf), "%.2f cm", dist2);
            gfx_puts(buf);
        }

        gfx_setCursor(10, 190);
        if (dist1 > 0 && dist2 > 0) {
            float diff = dist1 - dist2;


            gfx_puts("Dif 2: ");
            gfx_setCursor(10, 220);
            snprintf(buf, sizeof(buf), "%.2f cm", diff);
            gfx_puts(buf);
        } else {
            gfx_puts("Dif: ---");
        }
